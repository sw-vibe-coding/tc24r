...
DEMO_C="$SCRIPT_DIR/demo25.c"
DEMO_S="$SCRIPT_DIR/demo25.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 25: struct ==="
...
if echo "$UART" | grep -q "D25OK"; then echo "  [PASS] UART: D25OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 25 PASSED"; else echo "Demo 25 FAILED"; exit 1; fi
