...
DEMO_C="$SCRIPT_DIR/demo30.c"
DEMO_S="$SCRIPT_DIR/demo30.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 30: line continuation ==="
...
if echo "$UART" | grep -q "D30OK"; then echo "  [PASS] UART: D30OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 30 PASSED"; else echo "Demo 30 FAILED"; exit 1; fi
