...
if echo "$UART" | grep -q "D24OK"; then echo "  [PASS] UART: D24OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 24 PASSED"; else echo "Demo 24 FAILED"; exit 1; fi
