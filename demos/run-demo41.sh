...
DEMO_C="$SCRIPT_DIR/demo41.c"
DEMO_S="$SCRIPT_DIR/demo41.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 41: getc, atoi, string.h ==="
...
if echo "$UART" | grep -q "D41OK"; then echo "  [PASS] UART: D41OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 41 PASSED"; else echo "Demo 41 FAILED"; exit 1; fi
