...
if echo "$UART" | grep -q "D17OK"; then echo "  [PASS] UART: D17OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 17 PASSED"; else echo "Demo 17 FAILED"; exit 1; fi
