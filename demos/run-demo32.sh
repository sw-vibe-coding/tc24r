...
DEMO_C="$SCRIPT_DIR/demo32.c"
DEMO_S="$SCRIPT_DIR/demo32.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 32: multi-declarator typedef ==="
...
if echo "$UART" | grep -q "D32OK"; then echo "  [PASS] UART: D32OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 32 PASSED"; else echo "Demo 32 FAILED"; exit 1; fi
