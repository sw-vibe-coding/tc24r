...
DEMO_C="$SCRIPT_DIR/demo45.c"
DEMO_S="$SCRIPT_DIR/demo45.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 45: Lisp eval ==="
"$CC24" "$DEMO_C" -o "$DEMO_S" -I "$ROOT_DIR/include"
OUTPUT=$(cor24-run --run "$DEMO_S" --dump --speed 0 --time 120 2>&1)
echo "$OUTPUT"
echo ""
...
if echo "$UART" | grep -q "D45OK"; then echo "  [PASS] UART: D45OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 45 PASSED"; else echo "Demo 45 FAILED"; exit 1; fi
