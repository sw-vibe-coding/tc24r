...
DEMO_C="$SCRIPT_DIR/demo35.c"
DEMO_S="$SCRIPT_DIR/demo35.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 35: struct array members ==="
...
if echo "$UART" | grep -q "D35OK"; then echo "  [PASS] UART: D35OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 35 PASSED"; else echo "Demo 35 FAILED"; exit 1; fi
