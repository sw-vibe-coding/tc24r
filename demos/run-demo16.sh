...
if echo "$UART" | grep -q "D16OK"; then echo "  [PASS] UART: D16OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 16 PASSED"; else echo "Demo 16 FAILED"; exit 1; fi
