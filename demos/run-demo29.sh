...
DEMO_C="$SCRIPT_DIR/demo29.c"
DEMO_S="$SCRIPT_DIR/demo29.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 29: sizeof array types ==="
...
if echo "$UART" | grep -q "D29OK"; then echo "  [PASS] UART: D29OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 29 PASSED"; else echo "Demo 29 FAILED"; exit 1; fi
