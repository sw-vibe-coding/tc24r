...
DEMO_C="$SCRIPT_DIR/demo47.c"
DEMO_S="$SCRIPT_DIR/demo47.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 47: ptr[i].member — struct pointer indexing ==="
"$CC24" "$DEMO_C" -o "$DEMO_S" -I "$ROOT_DIR/include"
OUTPUT=$(cor24-run --run "$DEMO_S" --dump --speed 0 --time 30 2>&1)
echo "$OUTPUT"
echo ""
...
if [ "$R0" = "160" ]; then echo "  [PASS] r0 = 160"; else echo "  [FAIL] r0 = $R0"; PASS=false; fi
if echo "$UART" | grep -q "D47OK"; then echo "  [PASS] UART: D47OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 47 PASSED"; else echo "Demo 47 FAILED"; exit 1; fi
