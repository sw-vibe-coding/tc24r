...
DEMO_C="$SCRIPT_DIR/demo38.c"
DEMO_S="$SCRIPT_DIR/demo38.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 38: struct brace initializer ==="
...
if echo "$UART" | grep -q "D38OK"; then echo "  [PASS] UART: D38OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 38 PASSED"; else echo "Demo 38 FAILED"; exit 1; fi
