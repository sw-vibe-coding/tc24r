...
DEMO_C="$SCRIPT_DIR/demo27.c"
DEMO_S="$SCRIPT_DIR/demo27.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 27: function prototypes ==="
...
if echo "$UART" | grep -q "D27OK"; then echo "  [PASS] UART: D27OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 27 PASSED"; else echo "Demo 27 FAILED"; exit 1; fi
