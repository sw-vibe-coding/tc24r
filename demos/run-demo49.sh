...
DEMO_C="$SCRIPT_DIR/demo49.c"
DEMO_S="$SCRIPT_DIR/demo49.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 49: (ptr + offset)->member ==="
...
if echo "$UART" | grep -q "D49OK"; then echo "  [PASS] UART: D49OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 49 PASSED"; else echo "Demo 49 FAILED"; exit 1; fi
