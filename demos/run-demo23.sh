...
DEMO_C="$SCRIPT_DIR/demo23.c"
DEMO_S="$SCRIPT_DIR/demo23.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 23: enum ==="
...
if echo "$UART" | grep -q "D23OK"; then echo "  [PASS] UART: D23OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 23 PASSED"; else echo "Demo 23 FAILED"; exit 1; fi
