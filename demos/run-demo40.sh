...
DEMO_C="$SCRIPT_DIR/demo40.c"
DEMO_S="$SCRIPT_DIR/demo40.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 40: malloc/free ==="
...
if echo "$UART" | grep -q "D40OK"; then echo "  [PASS] UART: D40OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 40 PASSED"; else echo "Demo 40 FAILED"; exit 1; fi
