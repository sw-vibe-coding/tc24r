...
DEMO_C="$SCRIPT_DIR/demo37.c"
DEMO_S="$SCRIPT_DIR/demo37.s"
cargo build --manifest-path "$ROOT_DIR/components/cli/Cargo.toml" --release --quiet

echo "=== tc24r Demo 37: anonymous struct/union members ==="
...
if echo "$UART" | grep -q "D37OK"; then echo "  [PASS] UART: D37OK"; else echo "  [FAIL] UART: '$UART'"; PASS=false; fi
echo ""
if [ "$PASS" = true ]; then echo "Demo 37 PASSED"; else echo "Demo 37 FAILED"; exit 1; fi
